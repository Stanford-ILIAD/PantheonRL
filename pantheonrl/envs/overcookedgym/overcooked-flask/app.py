...
def predict():
    if request.method == 'POST':
        ...
        state_dict, player_id_dict, server_layout_name, algo, timestep = data_json["state"], data_json["npc_index"], data_json["layout_name"], data_json["algo"], data_json["timestep"]
        ...
        if ARGS.replay_traj:
            if player_id == 0:     a = int(EGO_TRANSITIONS.acts[timestep][0]) if timestep < len(EGO_TRANSITIONS.acts) else 4
            elif player_id == 1:   a = int(ALT_TRANSITIONS.acts[timestep][0]) if timestep < len(EGO_TRANSITIONS.acts) else 4
            else:                  assert(False)
        else:
            if player_id == 0:     s, policy = s0, POLICY_P0
            elif player_id == 1:   s, policy = s1, POLICY_P1
            else:                  assert(False)
...
def updatemodel():
    if request.method == 'POST':
        data_json = json.loads(request.data)
        traj_dict, traj_id, server_layout_name, algo = data_json["traj"], data_json["traj_id"], data_json["layout_name"], data_json["algo"]
        layout_name = NAME_TRANSLATION[server_layout_name]
        print(traj_id)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--modelpath_p0', type=str, help="path to load model for player 0")
    parser.add_argument('--modelpath_p1', type=str, help="path to load model for player 1")
    parser.add_argument('--replay_traj', type=str, help="replay traj, don't run policies")
    parser.add_argument('--layout_name', type=str, required=True, help="layout name")
    parser.add_argument('--trajs_savepath', type=str, help="path to save trajectories")
    ...
    if ARGS.replay_traj:
        ...
        simultaneous_transitions = SimultaneousTransitions.read_transition("%s.npy" % ARGS.replay_traj, env.observation_space, env.action_space)
        ...
        if ARGS.modelpath_p0: POLICY_P0 = PPO.load(ARGS.modelpath_p0)
        if ARGS.modelpath_p1: POLICY_P1 = PPO.load(ARGS.modelpath_p1)
